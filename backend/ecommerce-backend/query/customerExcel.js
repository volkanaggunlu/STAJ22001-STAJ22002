...
async function queryOrdersForExcel() {
    try {
        ...
        const orders = await Order.find({
            ...
            createdAt: { $gte: new Date('2025-04-02') }
        })
            .populate({
                path: 'cart',
                populate: {
        ...
        worksheet.columns = [
            ...
            { header: 'Sipariş', key: 'items', width: 35 },
        ...
        orders.forEach(order => {
            ...
            worksheet.addRow({
                ...
                // email: order.email,
                phone: order.phone,
                // concatenate product names with a comma, and if the product is a bundle extract the product names from the bundle
                items: order.cart.products.map(product => {
                    if (product.bundledProducts && product.bundledProducts.length > 0) {
                        return product.bundledProducts.map(bp => bp.name).join(', ');
                    } else {
...
// Execute the export
generateOrdersExcel('customer_excel.xlsx');